...
class MS0101M_adm_list(generics.ListAPIView):
    ...
    def list(self, request):
        ...
        query += " C.ms_name,B.pr_yr,ifnull(B.pr_sch_yr,'0') as pr_sch_yr, ifnull(B.pr_term_div,'0') as pr_term_div, ifnull(A.score1,'0') as score1, ifnull(A.score2,'0') as score2, ifnull(A.score3,'0') as score3, ifnull(A.score4,'0') as score4,A.* from service20_ms_apl A,service20_vw_nanum_stdt B,service20_msch C where A.apl_id=B.apl_id and A.ms_id = C.ms_id and A.yr='"+l_yr+"' and A.ms_id = '"+ms_ida+"' and A.apl_id='"+ida+"'"
...
class MP0102M_list(generics.ListAPIView):
    ...
    def list(self, request):
        ...
        query = f"""
                ...
                LEFT JOIN service20_mp_att t4 ON (t4.mp_id = t1.mp_id AND t4.apl_no = t2.apl_no and t4.spc_no = '1')
...
class MP0104M_list(generics.ListAPIView):
    ...
    def list(self, request):
        ...
        query += " , ifnull(sec_to_time(sum(time_to_sec(t2.elap_tm))), '00:00:00') sum_elap_tm  /* 경과시간*/ "
        query += " , ifnull(sum(t2.appr_tm), 0)   sum_appr_tm /* 인정시간*/ "
        query += " , ifnull(sum(t2.exp_amt), 0)   sum_exp_amt /* 지급 활동비 */"
        query += " , ifnull(sum(t2.appr_tm), 0)   cum_appr_tm /* 누적시간*/ "
        query += " , t1.bank_nm         /* 은행 명*/ "
        query += " , t1.bank_acct       /* 은행 계좌 번호*/ "
        query += " , t1.apl_id "
...
def MP01041M_Insert(request):
    ...
    query = f"""
            ...
            insert into service20_mp_att (
               ...
               , null
               , {l_appr_tm} * (SELECT s1.att_val
                                FROM service20_mp_sub s1
                                WHERE s1.mp_id   = '{l_mp_id}'
                                AND s1.att_id  = 'MP0032'
                                AND s1.att_cdh = 'MP0032'
                                AND s1.att_cdd = '00')
               , '{ins_id}'
               , '{client_ip}'
               , now()
...
def MP01041M_req(request):
    ...
    query += "   set mp_div = '" + str(l_mp_div) + "' "
    query += "     , att_sdt = concat('" + str(l_att_sdt) + "', ' " + str(l_att_stm_h) + "', ':', '" + str(l_att_stm_m) + "', ':00')"
    ...
    query += "     , exp_amt = " + str(l_appr_tm) + " * (SELECT s1.att_val FROM service20_mp_sub s1 WHERE s1.mp_id   = '" + str(l_mp_id) + "' AND s1.att_id  = 'MP0032' AND s1.att_cdh = 'MP0032' AND s1.att_cdd = '00')"
    # query += "     , appr_tm = '" + str(l_appr_tm) + "'"
...
# 출석(수정) 파일 업로드 ###################################################
@csrf_exempt
def MP01041M_upload_upd(request):
    req = request
    DIR = os.getcwd()
    UPLOAD_DIR = str(DIR) + '/media/MP01041M/'
    UPLOAD_DIR = '/NANUM/www/img/mp_attend/'
...
# 학습외 출석 여부 ###################################################
class MP01041M_att_cnt_Serializer(serializers.ModelSerializer):
    id = serializers.SerializerMethodField()
...
class MP01041M_att_cnt(generics.ListAPIView):
    queryset = mp_att.objects.all()
    serializer_class = MP01041M_att_cnt_Serializer

    def list(self, request):
        l_mp_id = request.GET.get('mp_id', "")
        l_apl_no = request.GET.get('apl_no', "")
        l_mp_div = request.GET.get('mp_div', "")
...
# 출석 추가 ###################################################
@csrf_exempt
def MP01041M_att_insert(request):
    l_mp_id = request.POST.get('u_mp_id', "")
    l_apl_no = request.POST.get('u_apl_no', "")
    l_att_no = request.POST.get('u_att_no', "")
    l_mp_div = request.POST.get('u_mp_div', "")
